...
export default class {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (!Array.isArray(splitData) || splitData.length !== 6) {
          ...
          [this.accountTopic, this.accountToken, , this.accountId, this.iotEndpoint, this.iotPass] = splitData;
          ...
          try {
            await this.storageData.setItem(
              ...
              `${this.accountTopic}:::${accountToken}:::${this.config.username}:::${this.accountId}:::${this.iotEndpoint}:::${this.iotPass}`,
            );
          } catch (e) {
  ...
  initialiseDevice(device) {
    ...
    try {
      ...
      delete accessory.context.isOnline;
    ...
    switch (params.cmd) {
      ...
      case 'scene': {
        /*
          ...
          AWS needs { cmd: 'ptReal', data: { command: code STR } }
          BLE we can try the same code as AWS
          LAN does not support this
        */

        // We get the scene in the non-base64 format
        // For AWS, we need to convert to base64
        // For BLE, we need to remove the first four and last two characters, then map each two digits to a hex value
        const awsCode = Buffer.from(params.value, 'hex').toString('base64');
        const bleCode = hexToTwoItems(params.value.substring(4, params.value.length - 2)).map((item) => `0x${item}`);
  ...
  receiveUpdateAWS(payload) {
    ...
    const toSend = {
      ...
      ...payload.state,
      op: payload.op,
      bulb: payload.cmd === 'bulb' && Array.isArray(payload.state.value)
        ? payload.state.value
        : undefined,
    };
    this.receiveDeviceUpdate(accessory, toSend);
  }
  ...
  receiveDeviceUpdate(accessory, params) {
    ...
    */
    if (hasProperty(params, 'onOff')) {
      if (platformConsts.models.switchDouble.includes(accessory.context.gvModel)) {
        ...
        data.state = [1, 17].includes(params.onOff) ? 'on' : 'off';
      }
    ...
    */
    if (params.color) {
      ...
      if (hasProperty(params.data, 'red')) {
        ...
        if (!platformConsts.awsColourShort.includes(accessory.context.gvModel) && !hasProperty(params.data, 'red')) {
          accessory.logWarn(`${platformLang.supportColorShort} [${accessory.context.gvModel}]`);
        }
      }
      ...
      if (
        ...
        accessory.logWarn(`${platformLang.supportColorWC} [${accessory.context.gvModel}]`);
      }
    ...
    */
    if (params.colorTemInKelvin) {
      ...
      data.kelvin = params.colorTem;
    } else if (params.cmd === 'colorTem') {
      data.kelvin = params.data.colorTemInKelvin;
